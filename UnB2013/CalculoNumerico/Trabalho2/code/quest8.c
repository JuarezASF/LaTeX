...
void SplitCurvature2D(const char *file, int M_data, \
									const char *curvature_save)
/* recebe :
 * -nome de um arquivo com os dados 
 * -número de dados a serem lidos
 * -nome de um arquivo aonde sairá a curvatura do split
 * 
 * *Faz isso em 2D, i.e.:
 * 					OS DADOS DEVEM TER DUAS COLUNAS
 * o que o código lerá são as duas primeiras colunas
...
{
	Matrix *D0, *D1, *DATA, *coeff0, *coeff1;
	double  DT, DDT, DY, DDY, curvature, T, s, ds;
	int i;
	FILE *fp;
	...
	fp = fopen(curvature_save, "w");
	
	ds = pow(10, -2);
	for(i = 0; i < coeff0->rows; i++)
		{
		for(s = 0 ; s <=1 ; s += ds)
			{
				//calculamos as derivas baseado nas fórmulas
				// e nos coeficients conhecidos
				DT = 3*coeff0->a[i][0]*pow(s, 2) + \
				     2*coeff0->a[i][1]*pow(s, 1) + \
				     1*coeff0->a[i][2]*pow(s, 0);
				     
				DDT = 6*coeff0->a[i][0]*pow(s, 1) + \
				     2*coeff0->a[i][1]*pow(s, 0);
				
				DY = 3*coeff1->a[i][0]*pow(s, 2) + \
				     2*coeff1->a[i][1]*pow(s, 1) + \
				     1*coeff1->a[i][2]*pow(s, 0);
				     
				DDY = 6*coeff1->a[i][0]*pow(s, 1) + \
				     2*coeff1->a[i][1]*pow(s, 0);
				
				T = coeff0->a[i][0]*pow(s, 3) + \
					coeff0->a[i][1]*pow(s, 2) + \
					coeff0->a[i][2]*pow(s, 1) + \
					coeff0->a[i][3];
				     
				curvature = absf(DT*DDY - DY*DDT)/ \
									pow(DT*DT + DY*DY, 3/2);
...
int main()
{
	...
	char 	curv_save[] = "./quest8-curvature.dat";
	
	int M_data = 17;
			
	SplitCurvature2D(file, M_data, curv_save);
	EndProgram();

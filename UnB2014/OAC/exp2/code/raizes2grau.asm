...
.data
	str_entrada : .asciiz "Consideramos o polin�mio p(x) = a*x^2 + b*x + c. \n\n"
...
read_pol: 
#recebe: nada
#faz: pede ao usuário a, b, c e os coloca em f0, f1 e f2 respectivamente
...
#faz: mostra polinômio
#altera registradores: nenhum
	#vamos usar a0 e v0, por isso salvamos na pilha
	#na verdade também vamos usar f12, ver como salvar isso na pilha depois
	addi $sp, $sp, -12
	sw $v0, 8($sp)
	sw $a0, 4($sp)	
	swc1 $f12, 0($sp)

	#imprime string de apresentação
	li $v0, 4
	la $a0, str_1
	syscall
	
	#imprime a
	li $v0, 2 # para imprimir float que está em $f12
	mov.s $f12, $f0
	syscall
...
#faz: calcula ra�zes de p(x) = a*x^2 + b*x + c e coloca os valores na pilha (single)
#retorna: $a0: 1 se ra�zes reais, nesse caso os valores est�o em 0(sp), 4(sp) 
#		2 se raízes complexas; nesse caso as ra�zes s�o 0(sp) + i*4(sp) e 8(sp) *i*12(sp)
#altera: a0
...
#recebe: raízes em f0, f1, f2, f3; forma das raízes em a0
...
#altera: nada
	...
	#decide qual modo de impress�o baseado em a0
	li $t0, 1

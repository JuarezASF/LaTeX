...
void PrintSplit3D(Matrix *DATA, Matrix *coeff0, Matrix *coeff1,\
									Matrix *coeff2,\
									const char *file)
{
	FILE *fp;
	...
	for (i=0; i < DATA->rows - 1; i++)
		...
		for (s = 0; s <= 1; s+= ds)
			...
			z = coeff2->a[i][0]*pow(s, 3) + \
				coeff2->a[i][1]*pow(s, 2) + \
				coeff2->a[i][2]*pow(s, 1) + \
				coeff2->a[i][3];			
...
Matrix *get_data_from_file3D(const char *file, int M_data)
 ...
 * */
{
		...
		for (n = 0; n < M_data; n++)
			fscanf(fp, "%lf %lf %lf", &DATA->a[n][0],\
			&DATA->a[n][1], &DATA->a[n][2]); 	
...
void SplitCalculate3D(const char *file, int M_data, \
 ...
 * */
{
	...
	DATA 	= get_data_from_file3D(file, M_data);
	...
	coeff2 = SolveCoeff(D2, DATA, 2);
	
	PrintSplit3D(DATA, coeff0, coeff1, coeff2, split_save);
...
int main()
{
	char 	file[] = "./../data/tab2.dat";
	char 	split[] = "./quest9-split3D.dat";
	
	int M_data = 12;
			
	SplitCalculate3D(file, M_data, split);
	EndProgram();
